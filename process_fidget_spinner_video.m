%Extracts angular velocity of fidget spinner from video
%INPUTS:
...
function [tlist,freq_list] = process_fidget_spinner_video(fname,varargin)
    T_window = 1;
    q = .6;
    showAnalysis = 1;
    ...
    [data_mat, Fs] = extract_data_from_video(fname);
    y = optimize_coefficients(data_mat);

    [tlist,freq_list] = fidget_spinner_FFT_complex(y,Fs,T_window,q,showAnalysis);
end


function y = optimize_coefficients(data_mat)
    ...
    y = Mt(:,1)./Mt_norm + sqrt(-1)*Mt(:,2)./Mt_norm;
...
%   tlist: list of times for measured values of angular frequency
%   omega_list: measured value of angular frequency
function [tlist,omega_list] = fidget_spinner_FFT_complex(y,Fs,T_window,q,showAnalysis)
    ...
    [tlist1,freq_list1] = extract_max_integer_freq_complex(y,Fs,N_window,showAnalysis);
...
function [tlist,freq_list] = extract_max_integer_freq_complex(y,Fs,N_window,showFFT) 
    ...
    fft_fig = []; omega_fig = [];
    window_left_plot1 = []; window_right_plot1 = [];
    window_left_plot2 = []; window_right_plot2 = [];
    ...
    if showFFT
        ...
        axis([0,tlist_base(end),0,N_window]);
        ...
        xlabel('time (sec)');
        ylabel('real(y(t)) (-)');
        plot(tlist_base,real(y),'k','linewidth',1);
        axis([min(tlist_base),max(tlist_base),-1.5,1.5]);
        window_left_plot1 = plot(0,0,'r','linewidth',1);
        window_right_plot1 = plot(0,0,'r','linewidth',1);
        ...
        xlabel('time (sec)');
        ylabel('imag(y(t)) (-)');
        plot(tlist_base,imag(y),'k','linewidth',1);
        axis([min(tlist_base),max(tlist_base),-1.5,1.5]);
        window_left_plot2 = plot(0,0,'r','linewidth',1);
        window_right_plot2 = plot(0,0,'r','linewidth',1);
    ...
    for n = 0:(Nsamples-N_window)
        ...
        if showFFT && mod(index0,2)==0
            ...
            set(window_left_plot1,'xdata',tlist_base(index0)*[1,1],'ydata',1.5*[-1,1]);
            set(window_right_plot1,'xdata',tlist_base(index1)*[1,1],'ydata',1.5*[-1,1]);
            set(window_left_plot2,'xdata',tlist_base(index0)*[1,1],'ydata',1.5*[-1,1]);
            set(window_right_plot2,'xdata',tlist_base(index1)*[1,1],'ydata',1.5*[-1,1]);
...
%by randomly choosing a set of pixel and then measuring
%their rgb values over time
%INPUTS:
%   fname: a string of the filename of the video you want to process
%           the the video is not in the same folder, fname should include
%           the absolute path to the file
...
function  [data_mat, Fs] = extract_data_from_video(fname)
    ...
    for n = 1:nFrames
        %compute progress in percent
        percent_complete = floor(100*n/nFrames);
